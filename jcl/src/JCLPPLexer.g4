...
SLASH : '/' ;
SQUOTE : '\'' ->channel(HIDDEN),pushMode(QS_MODE) ;
...
OP_PARM_SYMBOLIC : SYMBOLIC ->type(SYMBOLIC) ;
OP_PARM_CONTINUED : COMMA_DFLT NEWLINE ->channel(HIDDEN),pushMode(COMMA_NEWLINE_MODE) ;
OP_PARM_COMMENT_FLAG_INLINE : COMMENT_FLAG_INLINE ->channel(HIDDEN),pushMode(COMMA_WS_MODE) ;
OP_PARM_WS : [ ]+
    {
      _modeStack.clear();
    } ->channel(HIDDEN),mode(CM_MODE) ;
OP_PARM_NEWLINE : NEWLINE
    {
      _modeStack.clear();
    } ->channel(HIDDEN),mode(DEFAULT_MODE) ;
OP_PARM_COMMA : COMMA_DFLT ->type(COMMA),channel(HIDDEN) ;
OP_PARM_COMMENT_FLAG : COMMENT_FLAG_DFLT ->type(COMMENT_FLAG),channel(COMMENTS),pushMode(COMMA_NEWLINE_CM_MODE) ;
OP_PARM_SS_WS : SS ' '+
    {
      getText().length() <= 15
    }? ->channel(HIDDEN) ;
OP_PARM_SQUOTE : SQUOTE ->channel(HIDDEN),pushMode(QS_MODE) ;
OP_PARM_ANYCHAR : ANYCHAR ->channel(HIDDEN) ;
...
COMMAND_PARM_SQUOTE : SQUOTE ->channel(HIDDEN),pushMode(QS_MODE) ;
...
JCL_COMMAND_PARM_SQUOTE : SQUOTE ->channel(HIDDEN),pushMode(QS_MODE) ;
...
DD_ACCODE : A C C O D E ->type(ACCODE),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_AMP : A M P ->type(AMP),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_AVGREC : A V G R E C ->type(AVGREC),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_ASTERISK : '*'
    {
      ...
      dlmVals.add("//");
      myMode = DATA_MODE;
    } ->type(ASTERISK),channel(HIDDEN),pushMode(DATA_PARM_MODE) ;
DD_BLKSIZE : B L K S I Z E ->type(BLKSIZE),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_BLKSZLIM : B L K S Z L I M ->type(BLKSZLIM),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_BURST : B U R S T ->type(BURST),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_CCSID : C C S I D ->type(CCSID),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
...
DD_DATACLAS : D A T A C L A S ->type(DATACLAS),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
...
DD_FILEDATA : F I L E D A T A ->type(FILEDATA),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_FLASH : F L A S H ->type(FLASH),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_FREE : F R E E ->type(FREE),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_FREEVOL : F R E E V O L ->type(FREEVOL),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_GDGORDER : G D G O R D E R ->type(GDGORDER),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_HOLD : H O L D ->type(HOLD),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_KEYLABL1 : K E Y L A B L '1' ->type(KEYLABL1),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_KEYLABL2 : K E Y L A B L '2' ->type(KEYLABL2),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_KEYENCD1 : K E Y E N C D '1' ->type(KEYENCD1),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
...
DD_MAXGENS : M A X G E N S ->type(MAXGENS),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_MGMTCLAS : M G M T C L A S ->type(MGMTCLAS),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_MODIFY : M O D I F Y ->type(MODIFY),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
...
DD_REFDD : R E F D D ->type(REFDD),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_RETPD : R E T P D ->type(RETPD),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_RLS : R L S ->type(RLS),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
...
*/
DD_SEP : S E P ->type(SEP),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
...
DD_BUFIN : B U F I N ->type(BUFIN),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_BUFL : B U F L ->type(BUFL),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_BUFMAX : B U F M A X ->type(BUFMAX),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_BUFNO : B U F N O ->type(BUFNO),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_BUFOFF : B U F O F F ->type(BUFOFF),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_BUFOUT : B U F O U T ->type(BUFOUT),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_BUFSIZE : B U F S I Z E ->type(BUFSIZE),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_CPRI : C P R I ->type(CPRI),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_CYLOFL : C Y L O F L ->type(CYLOFL),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_DEN : D E N ->type(DEN),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_DIAGNS : D I A G N S ->type(DIAGNS),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_DSORG : D S O R G ->type(DSORG),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_EROPT : E R O P T ->type(EROPT),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_FUNC : F U N C ->type(FUNC),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_GNCP : G N C P ->type(GNCP),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_INTVL : I N T V L ->type(INTVL),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_IPLTXID : I P L T X I D ->type(IPLTXID),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_LIMCT : L I M C T ->type(LIMCT),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_MODE : M O D E ->type(MODE),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_NCP : N C P ->type(NCP),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_NTM : N T M ->type(NTM),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_OPTCD : O P T C D ->type(OPTCD),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_PCI : P C I ->type(PCI),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_PRTSP : P R T S P ->type(PRTSP),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_RESERVE : R E S E R V E ->type(RESERVE),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_RKP : R K P ->type(RKP),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
DD_STACK : S T A C K ->type(STACK),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
...
PROC_PARM_VALUE : SET_PARM_VALUE ->type(KEYWORD_VALUE) ;
...
XMIT_PARM_DEST : D E S T ->type(DEST),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
XMIT_PARM_DLM : DD_DLM ->type(DLM),channel(HIDDEN),pushMode(DLM_MODE) ;
XMIT_PARM_SUBCHAR : S U B C H A R S ->type(SUBCHARS),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
XMIT_PARM_NEWLINE : [\n\r] ->channel(HIDDEN),mode(DATA_MODE) ;
...
JOBGROUP_ACCT1_LPAREN : LPAREN_DFLT ->type(LPAREN),channel(HIDDEN),mode(JOBGROUP_ACCT2_MODE) ;
JOBGROUP_ACCT1_COMMA : COMMA_DFLT ->channel(HIDDEN),mode(JOBGROUP_PROGRAMMER_NAME_MODE) ;

JOBGROUP_EMAIL : E M A I L ->channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
JOBGROUP_GROUP : G R O U P ->channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
JOBGROUP_HOLD : H O L D ->channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
JOBGROUP_ERROR : E R R O R ->channel(HIDDEN),pushMode(JOBGROUP_ERROR_MODE) ;
JOBGROUP_ONERROR : O N E R R O R ->channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
JOBGROUP_OWNER : O W N E R ->channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
JOBGROUP_PASSWORD : P A S S W O R D ->channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
JOBGROUP_SECLABEL : S E C L A B E L ->channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
JOBGROUP_SCHENV : S C H E N V ->channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
JOBGROUP_SYSAFF : S Y S A F F ->channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
JOBGROUP_SYSTEM : S Y S T E M ->channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
JOBGROUP_TYPE : T Y P E ->channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;

JOBGROUP_ACCT1_SQUOTE : '\'' ->channel(HIDDEN),pushMode(QS_MODE) ;

JOBGROUP_ACCT_UNQUOTED_STRING : (~[,'\n\r] | SQUOTE2)+? ->channel(HIDDEN);
...
JOBGROUP_ACCT2_RPAREN : RPAREN_DFLT ->type(RPAREN),channel(HIDDEN),mode(JOBGROUP_ACCT3_MODE) ;

JOBGROUP_ACCT2_SQUOTE : '\'' ->channel(HIDDEN),pushMode(QS_MODE) ;
JOBGROUP_ACCT2_UNQUOTED_STRING : (~[,'\n\r] | SQUOTE2)+? ->type(JOBGROUP_ACCT_UNQUOTED_STRING),channel(HIDDEN) ;
...
JOBGROUP_PROGRAMMER_NAME_EMAIL : JOBGROUP_EMAIL ->type(JOBGROUP_EMAIL),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
JOBGROUP_PROGRAMMER_NAME_GROUP : JOBGROUP_GROUP ->type(JOBGROUP_GROUP),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
JOBGROUP_PROGRAMMER_NAME_HOLD : JOBGROUP_HOLD ->type(JOBGROUP_HOLD),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
JOBGROUP_PROGRAMMER_NAME_ERROR : JOBGROUP_ERROR ->type(JOBGROUP_ERROR),channel(HIDDEN),pushMode(JOBGROUP_ERROR_MODE) ;
JOBGROUP_PROGRAMMER_NAME_ONERROR : JOBGROUP_ONERROR ->type(JOBGROUP_ONERROR),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
JOBGROUP_PROGRAMMER_NAME_OWNER : JOBGROUP_OWNER ->type(JOBGROUP_OWNER),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
JOBGROUP_PROGRAMMER_NAME_PASSWORD : JOBGROUP_PASSWORD ->type(JOBGROUP_PASSWORD),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
JOBGROUP_PROGRAMMER_NAME_SCHENV : JOBGROUP_SCHENV ->type(JOBGROUP_SCHENV),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
JOBGROUP_PROGRAMMER_NAME_SECLABEL : JOBGROUP_SECLABEL ->type(JOBGROUP_SECLABEL),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
JOBGROUP_PROGRAMMER_NAME_SYSAFF : JOBGROUP_SYSAFF ->type(JOBGROUP_SYSAFF),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
JOBGROUP_PROGRAMMER_NAME_SYSTEM : JOBGROUP_SYSTEM ->type(JOBGROUP_SYSTEM),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
JOBGROUP_PROGRAMMER_NAME_TYPE : JOBGROUP_TYPE ->type(JOBGROUP_TYPE),channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;

JOBGROUP_PROGRAMMER_NAME_SQUOTE : '\'' ->channel(HIDDEN),pushMode(QS_MODE) ;
JOBGROUP_PROGRAMMER_NAME_UNQUOTED_STRING : (~[,'\n\r] | SQUOTE2)+? ->channel(HIDDEN) ;

mode JOBGROUP_ERROR_MODE ;

JOBGROUP_ERROR_EQUAL : EQUAL_DFLT ->type(EQUAL),channel(HIDDEN) ;
JOBGROUP_ERROR_LPAREN : LPAREN_DFLT ->type(LPAREN),channel(HIDDEN),mode(JOBGROUP_ERROR_PAREN_MODE) ;

mode JOBGROUP_ERROR_PAREN_MODE ;

JOBGROUP_ERROR_EQ : IF_EQ ->channel(HIDDEN) ;
JOBGROUP_ERROR_GE : IF_GE ->channel(HIDDEN) ;
JOBGROUP_ERROR_GT : IF_GT ->channel(HIDDEN) ;
JOBGROUP_ERROR_LE : IF_LE ->channel(HIDDEN) ;
JOBGROUP_ERROR_LT : IF_LT ->channel(HIDDEN) ;
JOBGROUP_ERROR_NE : IF_NE ->channel(HIDDEN) ;
JOBGROUP_ERROR_NG : IF_NG ->channel(HIDDEN) ;
JOBGROUP_ERROR_NL : IF_NL ->channel(HIDDEN) ;
JOBGROUP_ERROR_NOT : IF_NOT ->type(NOT_SYMBOL),channel(HIDDEN) ;

JOBGROUP_ERROR_ABEND : IF_ABEND ->type(ABEND),channel(HIDDEN) ;
JOBGROUP_ERROR_ABENDCC : IF_ABENDCC ->type(ABENDCC),channel(HIDDEN) ;
JOBGROUP_ERROR_FALSE : IF_FALSE ->type(FALSE),channel(HIDDEN) ;
JOBGROUP_ERROR_RC : IF_RC ->type(RC),channel(HIDDEN) ;
JOBGROUP_ERROR_RUN : IF_RUN ->type(RUN),channel(HIDDEN) ;
JOBGROUP_ERROR_TRUE : IF_TRUE ->type(TRUE),channel(HIDDEN) ;
JOBGROUP_ERROR_WS : WS ->channel(HIDDEN) ;
JOBGROUP_ERROR_NEWLINE : NEWLINE ->channel(HIDDEN) ;
JOBGROUP_ERROR_SS : SS ->channel(HIDDEN) ;
JOBGROUP_ERROR_LOGICAL : IF_LOGICAL ->channel(HIDDEN) ;
JOBGROUP_ERROR_PAREN_LPAREN : LPAREN_DFLT ->type(LPAREN),channel(HIDDEN),pushMode(JOBGROUP_ERROR_PAREN_MODE) ;
JOBGROUP_ERROR_RPAREN : RPAREN_DFLT ->type(RPAREN),channel(HIDDEN),popMode ;
JOBGROUP_ERROR_NUM_LIT : NUM_LIT_DFLT ->type(NUM_LIT),channel(HIDDEN) ;
JOBGROUP_ERROR_ALNUMNAT : ALNUMNAT ->type(ALNUMNAT),channel(HIDDEN) ;
...
GJOB_PARM_FLUSHTYP : F L U S H T Y P ->channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
...
JOBSET_PARM_FLUSHTYP : F L U S H T Y P ->channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
...
AFTER_PARM_NAME : N A M E ->channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
AFTER_PARM_WHEN : W H E N ->channel(HIDDEN),pushMode(JOBGROUP_ERROR_MODE) ;
AFTER_PARM_ACTION : A C T I O N ->channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
AFTER_PARM_OTHERWISE : O T H E R W I S E ->channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
...
BEFORE_PARM_NAME : N A M E ->channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
BEFORE_PARM_WHEN : W H E N ->channel(HIDDEN),pushMode(JOBGROUP_ERROR_MODE) ;
BEFORE_PARM_ACTION : A C T I O N ->channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
BEFORE_PARM_OTHERWISE : O T H E R W I S E ->channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
...
CONCURRENT_PARM_NAME : N A M E ->channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
...
JES2_JOBPARM_BURST : (B | (B U R S T))
    {
      hide = false;
    }
 ->pushMode(KYWD_VAL_MODE)
...
JES2_JOBPARM_BYTES : (M | (B Y T E S))
    {
      hide = false;
    }
 ->pushMode(KYWD_VAL_MODE)
...
JES2_JOBPARM_CARDS : (C | (C A R D S))
    {
      hide = false;
    }
 ->pushMode(KYWD_VAL_MODE)
...
JES2_JOBPARM_COPIES : (N | (C O P I E S))
    {
      hide = false;
    }
 ->pushMode(KYWD_VAL_MODE)
...
JES2_JOBPARM_FORMS : (F | (F O R M S))
    {
      hide = false;
    }
 ->pushMode(KYWD_VAL_MODE)
...
JES2_JOBPARM_LINECT : (K | (L I N E C T))
    {
      hide = false;
    }
 ->pushMode(KYWD_VAL_MODE)
...
JES2_JOBPARM_LINES : (L | (L I N E S))
    {
      hide = false;
    }
 ->pushMode(KYWD_VAL_MODE)
...
JES2_JOBPARM_PAGES : (G | (P A G E S))
    {
      hide = false;
    }
 ->pushMode(KYWD_VAL_MODE)
...
JES2_JOBPARM_PROCLIB : (P | (P R O C L I B))
    {
      hide = false;
    }
 ->pushMode(KYWD_VAL_MODE)
...
JES2_JOBPARM_RESTART : (E | (R E S T A R T))
    {
      hide = false;
    }
 ->pushMode(KYWD_VAL_MODE)
...
JES2_JOBPARM_SYSAFF : (S | (S Y S A F F))
    {
      hide = false;
    }
 ->pushMode(KYWD_VAL_MODE)
...
JES2_OUTPUT_CKPTLNS : (E | (C K P T L N S))
    {
      hide = false;
    }
 ->pushMode(KYWD_VAL_MODE)
...
JES2_OUTPUT_CKPTPGS : (P | (C K P T P G S))
    {
      hide = false;
    }
 ->pushMode(KYWD_VAL_MODE)
...
JES2_OUTPUT_COMPACT : (Z | (C O M P A C T))
    {
      hide = false;
    }
 ->pushMode(KYWD_VAL_MODE)
...
JES2_OUTPUT_COPIES : (N | (C O P I E S))
    {
      hide = false;
    }
 ->pushMode(KYWD_VAL_MODE)
...
JES2_OUTPUT_FLASHC : (Q | (F L A S H C))
    {
      hide = false;
    }
 ->pushMode(KYWD_VAL_MODE)
...
JES2_OUTPUT_LINDEX : (L | (L I N D E X))
    {
      hide = false;
    }
 ->pushMode(KYWD_VAL_MODE)
...
JES2_OUTPUT_LINECT : (K | (L I N E C T))
    {
      hide = false;
    }
 ->pushMode(KYWD_VAL_MODE)
...
JES2_OUTPUT_MODIFY : (Y | (M O D I F Y))
    {
      hide = false;
    }
 ->pushMode(KYWD_VAL_MODE)
...
JES2_OUTPUT_MODTRC : (M | (M O D T R C))
    {
      hide = false;
    }
 ->pushMode(KYWD_VAL_MODE)
...
JES2_XMIT_NODE : [A-Z0-9@#$*\-+&./%[:()]+ ->channel(HIDDEN) ;

mode JES2_XMIT_DLM_MODE ;

JES2_XMIT_DLM : DD_DLM ->type(DLM),channel(HIDDEN),pushMode(DLM_MODE) ;
...
JOB_ACCT_MODE1_SQUOTE : '\'' ->channel(HIDDEN),pushMode(QS_MODE) ;
...
JOB_ACCT_MODE2_SQUOTE : '\'' ->channel(HIDDEN),pushMode(QS_MODE) ;
...
JOB_PROGRAMMER_NAME_SQUOTE : '\'' ->channel(HIDDEN),pushMode(QS_MODE) ;
...
DISP_EQUAL : EQUAL_DFLT ->type(EQUAL),channel(HIDDEN) ;
DISP_SYMBOLIC : SYMBOLIC ->type(SYMBOLIC),popMode ;
DISP_LPAREN : LPAREN_DFLT ->type(LPAREN),channel(HIDDEN),pushMode(DISP_PAREN_MODE) ;

DISP_MOD : M O D ->channel(HIDDEN),popMode ;
DISP_NEW : N E W ->channel(HIDDEN),popMode ;
DISP_OLD : O L D ->channel(HIDDEN),popMode ;
DISP_SHR : S H R ->channel(HIDDEN),popMode ;

mode DISP_PAREN_MODE ;

DISP_PAREN_COMMA : COMMA_DFLT ->type(COMMA),channel(HIDDEN) ;
DISP_PAREN_MOD : DISP_MOD ->type(DISP_MOD),channel(HIDDEN) ;
DISP_PAREN_NEW : DISP_NEW ->type(DISP_NEW),channel(HIDDEN) ;
DISP_PAREN_OLD : DISP_OLD ->type(DISP_OLD),channel(HIDDEN) ;
DISP_PAREN_SHR : DISP_SHR ->type(DISP_SHR),channel(HIDDEN) ;
DISP_CATLG : C A T L G ->channel(HIDDEN) ;
DISP_DELETE : D E L E T E ->channel(HIDDEN) ;
DISP_KEEP : K E E P ->channel(HIDDEN) ;
DISP_PASS : P A S S ->channel(HIDDEN) ;
DISP_UNCATLG : U N C A T L G ->channel(HIDDEN) ;
DISP_PAREN_SYMBOLIC : SYMBOLIC ->type(SYMBOLIC) ;
DISP_PAREN_RPAREN : RPAREN_DFLT ->type(RPAREN),channel(HIDDEN),popMode,popMode ;
...
LABEL_SEQUENCE_SYMBOLIC : SYMBOLIC ->popMode ;
...
LABEL2_SEQUENCE_SYMBOLIC : SYMBOLIC ->type(LABEL_SEQUENCE_SYMBOLIC) ;
...
LABEL_PASSWORD_PROTECT_SYMBOLIC : SYMBOLIC ;
...
UNIT_SEP : S E P ->channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
...
UNIT_COUNT_SEP : S E P ->channel(HIDDEN),pushMode(KYWD_VAL_MODE) ;
...
*/
VOL_SEQ_NB_SER : VOL_SER ->type(VOL_SER),channel(HIDDEN),pushMode(VOL_SER4_MODE) ;
...
VOL_SER4_EQUAL : EQUAL_DFLT ->type(EQUAL),channel(HIDDEN) ;
VOL_SER4_NB : VOL_SER_NB ->type(VOL_SER_NB),channel(HIDDEN),popMode ;
VOL_SER4_SYMBOLIC : SYMBOLIC ->type(SYMBOLIC),popMode ;
VOL_SER4_SQUOTE : '\'' ->channel(HIDDEN),pushMode(QS_MODE) ;
VOL_SER4_LPAREN : LPAREN_DFLT ->type(LPAREN),channel(HIDDEN),pushMode(VOL_SER4_PAREN_MODE) ;

mode VOL_SER4_PAREN_MODE ;

VOL_SER4_PAREN : VOL_SER_NB ->type(VOL_SER_NB),channel(HIDDEN) ;
VOL_SER4_PAREN_SYMBOLIC : SYMBOLIC ->type(SYMBOLIC) ;
VOL_SER4_PAREN_SQUOTE : '\'' ->channel(HIDDEN),pushMode(QS_MODE) ;
VOL_SER4_PAREN_RPAREN : RPAREN_DFLT ->type(RPAREN),channel(HIDDEN),popMode,popMode ;
