...
//LASTRC : L A S T R C ;
LEAVE : L E A V E ;
LGSTREAM_DFLT : L G S T R E A M ->type(LGSTREAM) ;
...
*/
DATASET_NAME : (AMPERSAND AMPERSAND NAME) | ((AMPERSAND | NATL | ALPHA) (AMPERSAND | ALPHA | DOT | NATL | NUM)+) (LPAREN_DFLT ((HYPHEN? NUM+) | NAME | SYMBOLIC) RPAREN_DFLT)? ;
...
DD_DCB : DCB_DFLT ->type(DCB),mode(DEFAULT_MODE) ;
...
DD_DSN : DSN_DFLT ->type(DSN),mode(DEFAULT_MODE) ;
DD_DSNAME : DSNAME_DFLT ->type(DSNAME),mode(DEFAULT_MODE) ;
...
DD_SECMODEL : SECMODEL_DFLT ->type(SECMODEL),mode(DEFAULT_MODE) ;
