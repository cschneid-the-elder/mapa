...
DD_PATHDISP : P A T H D I S P ->type(PATHDISP),pushMode(KYWD_VAL_MODE) ;
DD_PATHMODE : P A T H M O D E ->type(PATHMODE),pushMode(KYWD_VAL_MODE) ;
DD_PATHOPTS : P A T H O P T S ->type(PATHOPTS),pushMode(KYWD_VAL_MODE) ;
...
DD_UCS : U C S ->type(UCS),pushMode(KYWD_VAL_MODE) ;
...
JOB_WS : [ ]+ ->channel(HIDDEN),mode(JOB_ACCT1_MODE) ;

mode JOB_ACCT1_MODE ;

JOB_ACCT1_MODE_NEWLINE : NEWLINE ->channel(HIDDEN),mode(DEFAULT_MODE) ;
JOB_ACCT1_MODE_WS : [ ]+ ->channel(HIDDEN),mode(CM_MODE) ;
JOB_ACCT1_MODE_COMMA_WS : COMMA_DFLT [ ]+ ->channel(HIDDEN),mode(JOB_ACCT_COMMA_WS_MODE) ;
JOB_ACCT1_MODE_COMMA_NEWLINE : COMMA_DFLT NEWLINE ->mode(JOB_ACCT_COMMA_NEWLINE_MODE) ;
JOB_ACCT1_MODE_LPAREN : LPAREN_DFLT ->type(LPAREN),mode(JOB_ACCT2_MODE) ;
JOB_ACCT1_MODE_COMMA : COMMA_DFLT ->type(COMMA),channel(HIDDEN),mode(JOB_PROGRAMMER_NAME_MODE) ;
...
JOB_ACCT1_MODE_SQUOTE : '\'' ->channel(HIDDEN),pushMode(QS_MODE) ;

JOB_ACCT1_MODE_UNQUOTED_STRING : (~[,'\n\r] | SQUOTE2)+? ->type(JOB_ACCT_MODE_UNQUOTED_STRING) ;
...
JOB_ACCT2_MODE_NEWLINE : NEWLINE ->channel(HIDDEN),mode(DEFAULT_MODE) ;
JOB_ACCT2_MODE_COMMA_WS : COMMA_DFLT [ ]+ ->channel(HIDDEN),pushMode(COMMA_WS_MODE) ;
JOB_ACCT2_MODE_RPAREN : RPAREN_DFLT ->type(RPAREN),mode(JOB_ACCT3_MODE) ;

JOB_ACCT2_MODE_SQUOTE : '\'' ->channel(HIDDEN),pushMode(QS_MODE) ;
JOB_ACCT2_MODE_UNQUOTED_STRING : (~[,'\n\r] | SQUOTE2)+? ->type(JOB_ACCT_MODE_UNQUOTED_STRING) ;

JOB_ACCT2_MODE_COMMA : COMMA_DFLT ->type(COMMA),channel(HIDDEN) ;

mode JOB_ACCT3_MODE ;

JOB_ACCT3_MODE_NEWLINE : NEWLINE ->channel(HIDDEN),mode(DEFAULT_MODE) ;
JOB_ACCT3_MODE_COMMA : COMMA_DFLT ->type(COMMA),channel(HIDDEN),mode(JOB_PROGRAMMER_NAME_MODE) ;
JOB_ACCT3_MODE_COMMA_WS : COMMA_DFLT ' '+ ->channel(HIDDEN),mode(JOB_ACCT_COMMA_WS_MODE) ;
JOB_ACCT3_MODE_COMMA_NEWLINE : COMMA_DFLT NEWLINE ->channel(HIDDEN),mode(JOB_ACCT_COMMA_WS_NEWLINE_MODE) ;
