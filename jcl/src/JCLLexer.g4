...
//QUOTED_STRING : SQUOTE (ANYCHAR_NOSQUOTE+ | SQUOTE2+ | (NEWLINE SS CONTINUATION_WS))* SQUOTE ;
//newline continuation doesn't work QUOTED_STRING : SQUOTE (ANYCHAR_NOSQUOTE | SQUOTE2)+ (NEWLINE SS CONTINUATION_WS (ANYCHAR_NOSQUOTE | SQUOTE2)+)* SQUOTE ;
//works as well as using SQUOTE QUOTED_STRING : '\'' (ANYCHAR_NOSQUOTE | SQUOTE2)+ (NEWLINE SS CONTINUATION_WS (ANYCHAR_NOSQUOTE | SQUOTE2)+)* '\'' ;
...
DATA_PARM_MODE_BLKSIZE : DCB EQUAL_DFLT BLKSIZE ;
DATA_PARM_MODE_BUFNO : DCB EQUAL_DFLT BUFNO ;
DATA_PARM_MODE_DIAGNS : DCB EQUAL_DFLT DIAGNS ;
DATA_PARM_MODE_LRECL : DCB EQUAL_DFLT LRECL ;
DATA_PARM_MODE_DLM : DLM EQUAL_DFLT ->type(DLM),pushMode(DLM_MODE) ;
...
DATA_PARM_MODE_MODEC : DCB EQUAL_DFLT MODE EQUAL_DFLT C ;
DATA_PARM_MODE_DSNAME : DSNAME ->type(DSNAME) ;
DATA_PARM_MODE_VOLSER : VOLUME EQUAL_DFLT SER ;
...
QUOTED_DLM_VAL : (ANYCHAR_NOSQUOTE+ | SQUOTE2+) ;
...
SQUOTE_QS : SQUOTE ->channel(HIDDEN),mode(DEFAULT_MODE) ;
ANYCHAR_NOSQUOTE : ~['\n\r] ;
NEWLINE_QS : [\n\r] ->channel(HIDDEN),mode(QS_SS) ;

QUOTED_STRING_FRAGMENT : (ANYCHAR_NOSQUOTE+ | SQUOTE2+) ;

mode QS_SS ;

SLASH_QS : '/' ;
SS_QS : SLASH_QS SLASH_QS {getCharPositionInLine() == 2}? ->channel(HIDDEN) ;
CONTINUATION_WS_QS_SS : ' '+ {getText().length() <= 13}? ->channel(HIDDEN),mode(QS) ;
